...
<table width="100%">
	...
	<input class="button_update" name = "useraction" type="submit" value="Update" >
...
<?php
	...
	if (isset($_POST['selectedrow']))
	{
		...
		foreach ($ary as $row){		 
			...
			foreach ($row as $key=>$value){
				...
				if ($key == 'reporturl') {$reporturl=$value;}		
				if ($key == 'reqref') {$reqref=$value;}					
	...
	echo '<input  maxlength="100" style="width:50%" type="text" name="reportid" value="'.$reportid.'" hidden>';
